...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Basari-ID/CRACK/blob/main/ua2.txt ko ko').text
		ua=open('.ua2.txt','w')
...
def banner():
	basari_tamvan('=' * 40)
	print(f"""{Z}[•] KELVINZMODZ [•]
{h}  ____  __  __ ____  ______ 
{h} |  _ \|  \/  |  _ \|  ____|
 {h}| |_) | \  / | |_) | |__   
{h} |  _ <| |\/| |  _ <|  __|  
{h} | |_) | |  | | |_) | |     
{h} 𝗸𝞔𝙇𝘃𝜤𝙉𝙕𝞛𝙊𝙙𝙕 {O}V.9999     {P}""")
	basari_tamvan('=' * 40)
...
def login_bas():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL ! JALANKAN LAGI PERINTAHNYA BG VINZ !{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
if __name__=='__main__':
	...
	basari_tamvan(f'\n\t{x} {h}Halo Selamat datang di sc kelvinzmodz :v{x}')
	time.sleep(3)
	login()
